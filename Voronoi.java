...
public class Voronoi {
	...
	private Point relaxPointLloyd(Point site) {
		double sumX = 0;
		double sumY = 0;
		double counter = 0;
		for (Edge e : edges) {
			if (e.site_left.equals(site)) {
				sumX += e.start.x;
				sumY += e.start.y;
				counter += 1;
			}
		}
		return new Point(sumX/counter % 1000, sumY/counter % 1000);
	}

	private void realEndEdges() {
		for (Edge e : edges) {
			if (e.start.x < 1 && e.start.y > 1) {
				Point start = getEdgeIntersection(e,
						new Edge(new Point(0, 1),
								 new Point(1, 1),
								 new Point(0, 0),
								 new Point(0, 1)));
			...
			if (e.end.x < 1 && e.end.y > 1) {
				Point end = getEdgeIntersection(e,
						new Edge(new Point(0, 1),
								new Point(1, 1),
								new Point(0, 0),
								new Point(0, 1)));
				System.out.println("Edge - " + e);
				System.out.println("Intersection end= " + end);
				e.end = end;
			}
		}
	}
...
//		points.add(new Point(1, 0.5));
		...
		System.out.println(diagram.edges);

		diagram.realEndEdges();

		ArrayList<Point> lloydPoints = new ArrayList<>();
		for (Point p : points) {
			lloydPoints.add(diagram.relaxPointLloyd(p));
		}

		Voronoi lldiagram = new Voronoi (lloydPoints);

		for (int i = 0; i < 2; i ++) {
			ArrayList<Point> prevPoints = (ArrayList<Point>) lldiagram.sites;
			lloydPoints = new ArrayList<Point>();
			for (Point p : prevPoints) {
				lloydPoints.add(lldiagram.relaxPointLloyd(p));
			}
			lldiagram = new Voronoi (lloydPoints);
		}

		//System.out.println(lldiagram.edges);
		...
		for (Point p: lldiagram.sites) {
			StdDraw.point(p.x, p.y);
		}

		StdDraw.setPenRadius(.002);
		for (Edge e: lldiagram.edges) {
			StdDraw.line(e.start.x, e.start.y, e.end.x, e.end.y);
		}
	}
}
